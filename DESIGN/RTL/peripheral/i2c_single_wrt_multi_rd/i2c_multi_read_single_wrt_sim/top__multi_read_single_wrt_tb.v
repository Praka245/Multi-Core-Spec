...
module tb_probe;
   ...
   // wire [2:0] byte_cnt;
    ...
    initial begin
        ...
        @(posedge clk); we=1; addr=7'h4; wdata = {24'd0, 7'h3C, 1'b0}; @(posedge clk); we=0;
        // set tx_data (register pointer) = 8'hA0
        @(posedge clk); we=1; addr=7'h5; wdata = 32'h0000002D; @(posedge clk); we=0;
       // @(posedge clk); we=1; addr=7'h8; wdata = 32'h0000002D; @(posedge clk); we=0;
        // kick off transaction
        @(posedge clk); we=1; addr=7'h9; wdata = 32'h00000008; @(posedge clk); we=0;
